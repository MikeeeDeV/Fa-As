...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // ---- Local File Protocol Routing Fix ----
  if (window.location.protocol === 'file:') {
    document.querySelectorAll('a[href]').forEach(function (a) {
      const href = a.getAttribute('href');
      if (href && !href.startsWith('http') && !href.startsWith('https://') && !href.startsWith('#') && !href.startsWith('javascript:') && !href.endsWith('.html')) {
        const parts = href.split('#');
        const newHref = parts[0] + '.html' + (parts[1] ? '#' + parts[1] : '');
  ...
  function initDarkMode() {
    ...
    document.querySelectorAll('.theme-toggle').forEach(function (btn) {
      btn.addEventListener('click', function (e) {
        ...
        showToast(newTheme === 'dark' ? 'تم تفعيل الوضع الليلي 🌙' : 'تم تفعيل الوضع النهارى ☀️');
      });
    });
  }
  ...
  window.showToast = function (msg, duration = 3000) {
    let toast = document.getElementById('pharma-toast');
    if (!toast) {
      toast = document.createElement('div');
      toast.id = 'pharma-toast';
      toast.style.cssText = 'position:fixed;bottom:30px;left:50%;transform:translateX(-50%) translateY(100px);background:var(--primary-dark);color:#fff;padding:12px 24px;border-radius:30px;font-family:Tajawal,sans-serif;font-size:14px;box-shadow:0 10px 30px rgba(0,0,0,0.3);z-index:99999;transition:transform 0.4s ease, opacity 0.4s ease;opacity:0;pointer-events:none;display:flex;align-items:center;gap:10px;border:1px solid rgba(255,255,255,0.2);';
      document.body.appendChild(toast);
    }
    toast.innerHTML = `<i class="fas fa-info-circle" style="color:var(--secondary)"></i> ${msg}`;
  ...
  // ---- Download PDF Triggers Notification ----
  document.querySelectorAll('a[download], .btn-download').forEach(function (btn) {
    btn.addEventListener('click', function () {
      showToast('جاري بدء تحميل الملف المطلوب 📄');
    });
  });
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', function (e) {
      e.preventDefault();
      const name = (document.getElementById('contactName') || document.getElementById('firstName'))?.value || 'الزائر الكريم';
      ...
      setTimeout(() => {
        ...
        showToast(`شكراً لك يا ${name}! تم استلام رسالتك وسيتواصل معك الفريق الأكاديمي قريباً.`);
        this.reset();
        setTimeout(() => {
  ...
  // ---- Image Error Safety Fallback ----
  document.querySelectorAll('img').forEach(function (img) {
    img.addEventListener('error', function () {
      if (!this.dataset.fallbackApplied) {
        this.dataset.fallbackApplied = 'true';
        this.src = 'pharma-as logo.png';
  ...
  console.log('كلية الصيدلة - جامعة الأزهر - أسيوط | الموقع محمّل ومطور بنجاح ✓');
});
